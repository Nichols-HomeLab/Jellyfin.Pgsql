...
public sealed class PostgreSqlProviderTests : IAsyncLifetime
{
    ...
    private PgSqlDatabaseProvider? _provider;
    ...
    public async Task Crud_BaseItem_Chapter_MediaStream()
    {
        ...
        await using (ctx)
        {
            ...
            // Create BaseItem
            var item = new BaseItemEntity { Id = itemId, Type = "Movie", Name = "Test Movie", OriginalLanguage = "eng" };
            ...
            var stream = new MediaStreamInfo { ItemId = itemId, Item = item, StreamIndex = 0, StreamType = MediaStreamTypeEntity.Video, IsOriginal = true };
            ctx.MediaStreamInfos.Add(stream);

            await ctx.SaveChangesAsync();

            // Read from the database rather than the tracked objects.
            ...
            Assert.Equal("eng", readItem.OriginalLanguage);
            Assert.True(Assert.Single(readItem.MediaStreams!).IsOriginal);
    ...
    /// <summary>
    /// Verifies that <see cref="PgSqlDatabaseProvider.RunScheduledOptimisation"/> executes ANALYZE without error.
    /// </summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Fact]
    ...
    /// <summary>
    /// Verifies that <see cref="PgSqlDatabaseProvider.PurgeDatabase"/> empties tables and resets <c>session_replication_role</c>.
    /// </summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
    [Fact]
    ...
    private JellyfinDbContext CreateContext()
    {
        ...
        optionsBuilder.UseNpgsql(_dataSource!, options => options.MigrationsAssembly(typeof(PgSqlDatabaseProvider).Assembly.FullName));
